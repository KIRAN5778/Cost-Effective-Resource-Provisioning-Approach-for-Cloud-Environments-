...
public class AmazonCloudWatchMonitor extends AManagedMonitor {
	...
	public Logger logger = Logger.getLogger(AmazonCloudWatchMonitor.class);
	...
	static {
		...
		tmpRegionVsURLs.put("ap-southeast-2",
				"monitoring.ap-southeast-2.amazonaws.com");
		tmpRegionVsURLs.put("ap-northeast-1",
	...
	private static final Map<String, String> DEFAULT_ARGS = new HashMap<String, String>() {
		{
			// put("configurations",
			// "monitors/CloudWatchMonitor/conf/AWSConfigurations.xml");
			// put("configurations", ServerConstants.project_url
			// + "/monitor.xml");

			put(TaskInputArgs.METRIC_PREFIX,
	...
	public void initialize(Map<String, String> taskArguments) throws Exception {
		if (!isInitialized) {
			...
			ca.setName("Console");
			// %-5level %logger{35}
			ca.setLayout(new PatternLayout("%d - %m %n"));
			ca.setWriter(new FileWriter(new File("a_log")));
			logger.addAppender(ca);
			taskArguments = ArgumentsValidator.validateArguments(taskArguments,
					DEFAULT_ARGS);
			metric_prefix = taskArguments.get(TaskInputArgs.METRIC_PREFIX);
			configuration = ConfigurationUtil.getConfigurations(taskArguments
					.get("configurations"));
	...
	private void setProxyParams(Configuration configuration) {
		if (!configuration.proxyParams.isEmpty()) {
			clientConfiguration.setProxyHost(configuration.proxyParams
					.get("proxyHost"));
			clientConfiguration.setProxyPort(Integer
					.parseInt(configuration.proxyParams.get("proxyPort")));
			clientConfiguration.setProxyUsername(configuration.proxyParams
					.get("proxyUserName"));
			clientConfiguration.setProxyPassword(configuration.proxyParams
					.get("proxyPassword"));
		}
	}
	...
	private void initializeEC2InstanceNameManager() {
		if (useEc2InstanceNameInMetrics
				&& availableNamespaces.contains(AWS_EC2_NAMESPACE)) {
			ec2InstanceNameManager = new EC2InstanceNameManager(credentials,
					configuration.tagFilterName, configuration.tagKey);
	 ...
	 */
	public void printMetric(String region, String namespacePrefix,
			String metricName, double metricValue, String aggregation,
			String timeRollup, String cluster) {
		try {
			String value = new BigDecimal(metricValue).setScale(0,
					RoundingMode.HALF_UP).toString();
			MetricWriter metricWriter = getMetricWriter(getMetricPrefix()
					+ region + namespacePrefix + metricName, aggregation,
					timeRollup, cluster);
			if (logger.isDebugEnabled()) {
				logger.debug("Metric: " + getMetricPrefix() + region
						+ namespacePrefix + metricName + " value: "
						+ metricValue + " -> " + value);
			}
	...
	public static String getImplementationVersion() {
		return AmazonCloudWatchMonitor.class.getPackage()
